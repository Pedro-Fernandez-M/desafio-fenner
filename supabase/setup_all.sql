...
-- =============================================================================
-- Antes: los puntos de profesores se aplicaban solo el viernes (consolidación).
-- Ahora: se aplican al instante como promedio (igual que convivencia), para que
-- el ranking refleje los registros de inmediato. Registrar varias veces no
-- infla porque es un promedio por indicador.
-- =============================================================================
...
begin
  ...
  on conflict (course_id, semester_id, week_number) do update
    set teacher_points = excluded.teacher_points,
        ...
        total_points   = excluded.teacher_points + excluded.conviv_points,
        updated_at     = now()
  returning conviv_posted, teacher_consolidated into v_old_conviv, v_old_teach;

  -- Ambos grupos en vivo: publica el delta total (puntaje doble: general = 2×xp)
  v_delta := (v_ct - coalesce(v_old_conviv, 0)) + (v_tt - coalesce(v_old_teach, 0));
  if v_delta <> 0 then
    insert into public.score_events
      (course_id, semester_id, type, general_delta, xp_delta, description,
       reference_table, reference_id, created_by)
    values
      (p_course_id, v_sem.id, 'evaluacion', v_delta * 2, v_delta,
       'Registros semana ' || v_week, 'class_week_totals', null, v_uid);
    update public.class_week_totals
      set conviv_posted = v_ct, teacher_consolidated = v_tt
      where course_id = p_course_id and semester_id = v_sem.id and week_number = v_week;
  end if;
...
-- =============================================================================
--   * user_indicators: restringe qué indicadores puede registrar un usuario.
--     Si un usuario tiene filas aquí, SOLO puede registrar esos indicadores.
--   * profiles.allowed_modules: si está definido, reemplaza el acceso por rol
--     (lista exacta de módulos que ve ese usuario).
--   * Convivencia puede registrar bonos.
-- =============================================================================
...
drop policy if exists user_indicators_select on public.user_indicators;
create policy user_indicators_select on public.user_indicators
  for select to authenticated using (true);
drop policy if exists user_indicators_admin on public.user_indicators;
create policy user_indicators_admin on public.user_indicators
  for all to authenticated using (public.is_admin()) with check (public.is_admin());

-- Convivencia también puede registrar bonos
drop policy if exists bonuses_insert on public.bonuses;
create policy bonuses_insert on public.bonuses
  for insert to authenticated with check (
    public.current_role_name() in ('administrador','direccion','convivencia')
  );

-- ---------------------------------------------------------------------------
-- submit_class_evaluation: agrega restricción por indicador (user_indicators)
-- ---------------------------------------------------------------------------
create or replace function public.submit_class_evaluation(
  p_course_id  uuid,
  p_class_date date,
...
begin
  ...
  -- Restricción por indicador (si el usuario tiene asignación específica)
  if exists (select 1 from public.user_indicators where user_id = v_uid) then
    if exists (
      select 1 from jsonb_array_elements(p_scores) s
      where (s->>'indicator_id')::uuid not in (
        select indicator_id from public.user_indicators where user_id = v_uid
      )
    ) then
      raise exception 'No tienes permiso para registrar alguno de esos indicadores';
    end if;
  end if;
